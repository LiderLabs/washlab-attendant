...
// ── Per-load pricing map (mirrors backend calculateServicePrice) ──────────────
// Update these values if your backend service prices change.
...
const KG_PER_LOAD = 8          // one load = 8 kg
const WHITES_EXTRA_LOAD = 1    // separate whites = 1 extra load
...
export function OnlineOrdersContent() {
  ...
  const allOnlineOrdersResult = usePaginatedQuery(
    ...
    stationToken
      ? { stationToken, orderType: "online" as any }
      : "skip",
    { initialNumItems: 50 }
  )
  ...
  const pendingOrders = (allOnlineOrdersResult.results || [])
    ...
    // ✅ Oldest first (lowest createdAt at the top)
  ...
  // Branch services (for accurate per-load pricing)
  ...
  const handleRejectConfirm = async (
    ...
    try {
      ...
      setWeight(0); setLaundryBags(1); setBagCardNumber(""); setNotes("")
    } catch (error) {
  ...
  const handleProceedToPayment = async () => {
    ...
    if (takenNumbers.has(bagCardNumber)) {
      toast.error(`Bag number ${bagCardNumber} is already assigned to an active order`)
      return
    }
    const orderId = selectedOrder._id
    const isAlreadyCheckedIn = (selectedOrder as any).status === "checked_in"
    try {
  ...
  // ── Pricing breakdown ────────────────────────────────────────────────────────
  // Try to get actual per-load price from branch services first,
  // fall back to the hardcoded map, and finally fall back to branchInfo.pricingPerKg.
  ...
  const getPricingBreakdown = () => {
    if (!selectedOrder || weight <= 0) {
      return {
        ...
        basePrice: 0,
        deliveryFee: 0,
        total: selectedOrder?.finalPrice || 0,
      }
    }

    const serviceType   = selectedOrder.serviceType || "wash_and_dry"
    const pricePerLoad  = getPricePerLoad(serviceType)
    const numberOfLoads = Math.ceil(weight / KG_PER_LOAD)
    const whitesExtraLoad = (selectedOrder as any).whitesSeparate ? WHITES_EXTRA_LOAD : 0
    ...
    return { numberOfLoads, whitesExtraLoad, totalLoads, pricePerLoad, basePrice, deliveryFee, total: basePrice + deliveryFee }
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-73px)] overflow-hidden">
      ...
      {/* ── Left: Queue ────────────────────────────────────────────────────── */}
      <div className="w-full lg:w-72 border-r border-border bg-card overflow-y-auto flex-shrink-0">
        <div className="p-4 border-b border-border">
          ...
          {/* ✅ Only estimated volume — oldest order stat removed */}
        ...
        {/* ✅ Order list — oldest at top, newest at bottom */}
            ...
            filteredOrders.map((order) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  {order.isDelivery && (
                    <><span>•</span><span className="text-warning">Delivery</span></>
                  )}
                </div>
              </button>
            ))
        ...
        <div className="flex-1 overflow-y-auto min-w-0 flex flex-col relative">
            ...
            <div className="flex flex-col gap-4 mb-4 sm:mb-6">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-muted-foreground mt-1 flex-wrap">
                    <span className="flex items-center gap-1">
                      ...
                      <span className="truncate">{selectedOrder.customer?.phoneNumber || "N/A"}</span>
                    </span>
              ...
              <div className="flex items-center justify-between sm:justify-start sm:gap-4">
                <div>
                  ...
                  <p className="font-bold text-foreground text-sm sm:text-base">#{selectedOrder.orderNumber}</p>
                </div>
                {bagCardNumber && (
                  <div className="text-right sm:text-left">
                    <p className="text-xs text-muted-foreground">Bag Card</p>
                    <p className="font-bold text-primary text-sm sm:text-base">#{bagCardNumber}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-sm text-muted-foreground">
                    Customer Estimate:{" "}
                    <strong>{selectedOrder.estimatedWeight?.toFixed(1) || "0.0"} kg</strong>
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-center">
                  <Button variant="outline" size="sm" onClick={() => setWeight(prev => Math.max(0, prev + 0.5))} className="w-full sm:w-auto">
                    +0.5 kg Bag Weight
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => setWeight(0)} className="w-full sm:w-auto">
                    Reset Scale
                  </Button>
                </div>
              </div>
          ...
          <div className="fixed lg:sticky bottom-0 left-0 right-0 lg:left-auto lg:right-auto bg-card border-t border-border p-3 sm:p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 z-50 shadow-lg lg:shadow-none">
            <div className="flex gap-2 sm:gap-3">
              <Button variant="outline" className="text-destructive border-destructive/30 flex-1 sm:flex-initial" onClick={handleRejectClick} size="sm">
                <Trash2 className="w-4 h-4 sm:mr-2" />
                <span className="text-xs sm:text-sm">Reject</span>
              </Button>
              <Button variant="outline" onClick={handleContact} size="sm" className="flex-1 sm:flex-initial">
