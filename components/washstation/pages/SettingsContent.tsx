...
export function SettingsContent() {
  ...
  // Fetch station info for loggedInAt (getStationInfo has loggedInAt)
      ...
      <div className='bg-card border border-border rounded-xl p-6 shadow-sm'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          <div className='flex items-center gap-3 p-4 rounded-lg bg-muted/50'>
            ...
            >
              <Printer
                className={`w-5 h-5 ${printerConnected ? "text-success" : "text-destructive"}`}
              />
            </div>
            <div>
              <p className='font-medium text-foreground'>Receipt Printer</p>
              <p
                className={`text-sm ${printerConnected ? "text-success" : "text-destructive"}`}
              >
