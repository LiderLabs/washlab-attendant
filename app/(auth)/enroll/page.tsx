...
export default function EnrollmentQueryPage() {
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-lg border-destructive/50">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl text-destructive">Invalid Enrollment Link</CardTitle>
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-lg">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl">Verifying Enrollment Link</CardTitle>
              <CardDescription className="mt-2">
  ...
  if (!tokenVerification.valid) {
    const errorMessage = tokenVerification.error || 'The enrollment link is invalid or has expired.';
    const isExpired = errorMessage.toLowerCase().includes('expired');
    const isAlreadyEnrolled = errorMessage.toLowerCase().includes('already enrolled');
    
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className={`max-w-md w-full shadow-lg ${isAlreadyEnrolled ? 'border-green-500/50' : 'border-destructive/50'}`}>
          <CardHeader className="text-center space-y-4">
            <div className="flex justify-center">
              <Logo className="h-12 w-auto" />
            </div>
            <div>
              <CardTitle className={`text-2xl ${isAlreadyEnrolled ? 'text-green-600' : 'text-destructive'}`}>
                {isAlreadyEnrolled ? 'Already Enrolled' : isExpired ? 'Enrollment Link Expired' : 'Invalid Enrollment Link'}
              </CardTitle>
              <CardDescription className="mt-2">
                {errorMessage}
              </CardDescription>
          ...
          <CardContent className="space-y-4">
            ...
            {isExpired && (
              <Alert>
                ...
                <AlertDescription>
                  Enrollment links expire after a set period for security reasons. 
                  Please contact your administrator to request a new enrollment link.
                </AlertDescription>
              </Alert>
            )}
            {isAlreadyEnrolled && (
            ...
            <div className="bg-muted/50 rounded-lg p-4 space-y-3 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <div className="text-muted-foreground">
                  ...
                  <span className="text-foreground">{tokenVerification.attendant?.email}</span>
                </div>
                <div className="text-muted-foreground">
                  <span className="font-medium">Branch:</span>{' '}
                  <span className="text-foreground">{tokenVerification.attendant?.branchName}</span>
                </div>
              </div>
              <div className={`text-xs flex items-center justify-center gap-1 ${isExpiringSoon ? 'text-destructive' : 'text-muted-foreground'}`}>
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Your biometric data is encrypted and stored securely using AES-256-GCM encryption. 
                  This information is used solely for identity verification and cannot be used to 
                  reconstruct your physical features. We follow industry-standard security practices 
